...
public class ProdAceite extends Producto {
    ...
    /**
     * Constructor de la clase ProdAceite.
     */
    public ProdAceite(Persona persona, double extension, Database database) {
        super("Aceite", persona, false, extension);
        ProdAceite.rendimiento = 1.25;      // Se necesitan ~4.5kg para 1l de aceite
        ProdAceite.valorPorKg = 6.36;       // Asumimos que kg = l aunque no sea real
    ...
    public static void actualizarValores(double nuevoValorPorKg, Database database) {           // Habrá que meterlo en una funcion de database!!!!!
        valorPorKgHistorico.put(database.getAnno(), ProdAceite.getValorPorKg());                  // Ojo al getAnno, tiene que ser el año correcto!!!!!
        ProdAceite.setValorPorKg(nuevoValorPorKg);
    }
    ...
    public static void setRendimiento(double rendimiento) {
        if(rendimiento <= 0) {
            ProdAceite.rendimiento = 0;
        } else {
            ProdAceite.rendimiento = rendimiento;
        }
    }
    ...
    public static void setValorPorKg(double valorPorKg) {
        if(valorPorKg <= 0) {
            ProdAceite.valorPorKg = 0;
        } else {
            ProdAceite.valorPorKg = valorPorKg;
        }
    }
